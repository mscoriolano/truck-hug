...
serve(async (req) => {
  ...
  try {
    ...
    // RequestReferenciaEntrega - Referências de entrega
    const xmlRequest = `<?xml version="1.0" encoding="UTF-8"?>
<RequestReferenciaEntrega>
  <login>${escapeXml(TRUCKSCONTROL_USER)}</login>
  <senha>${escapeXml(TRUCKSCONTROL_PASSWORD)}</senha>
</RequestReferenciaEntrega>`;

    console.log("[truckscontrol-referencia-entrega] start", { ts: new Date().toISOString() });
    ...
    const refNodes = parseXmlArray(responseText, "ReferenciaEntrega");
    const referencias = refNodes.map(xml => ({
      refID: parseXmlValue(xml, "refID"),
      veiID: parseXmlValue(xml, "veiID"),
      ref: parseXmlValue(xml, "ref"),
      dt: parseXmlValue(xml, "dt"),
      status: parseXmlValue(xml, "status"),
    }));

    console.log("[truckscontrol-referencia-entrega] finished", { count: referencias.length });
    ...
    console.error("[truckscontrol-referencia-entrega] error:", err);
