...
serve(async (req) => {
  ...
  try {
    ...
    if (input.action === "messages") {
      // RequestMensagemSpy - Mensagens dos equipamentos Spy
      xmlRequest = `<?xml version="1.0" encoding="UTF-8"?>
<RequestMensagemSpy>
  <login>${escapeXml(TRUCKSCONTROL_USER)}</login>
  <senha>${escapeXml(TRUCKSCONTROL_PASSWORD)}</senha>`;
      if (input.spyID) {
        xmlRequest += `\n  <spyID>${escapeXml(input.spyID)}</spyID>`;
      }
      xmlRequest += `\n</RequestMensagemSpy>`;
    } else {
      // RequestSpy - Lista de equipamentos Spy
      xmlRequest = `<?xml version="1.0" encoding="UTF-8"?>
<RequestSpy>
  <login>${escapeXml(TRUCKSCONTROL_USER)}</login>
  <senha>${escapeXml(TRUCKSCONTROL_PASSWORD)}</senha>
    ...
    console.log("[truckscontrol-spy]", { action: input.action, ts: new Date().toISOString() });
    ...
    if (input.action === "messages") {
      const msgNodes = parseXmlArray(responseText, "MensagemSpy");
      const mensagens = msgNodes.map(xml => ({
        mID: parseXmlValue(xml, "mID"),
        spyID: parseXmlValue(xml, "spyID"),
        latitude: parseXmlValue(xml, "latitude"),
        longitude: parseXmlValue(xml, "longitude"),
        dataHora: parseXmlValue(xml, "dataHora"),
        evento: parseXmlValue(xml, "evento"),
      }));
    ...
    const spyNodes = parseXmlArray(responseText, "Spy");
    const spys = spyNodes.map(xml => ({
      spyID: parseXmlValue(xml, "spyID"),
      serie: parseXmlValue(xml, "serie"),
      desc: parseXmlValue(xml, "desc"),
      ativo: parseXmlValue(xml, "ativo"),
    }));
    ...
    console.error("[truckscontrol-spy] error:", err);
