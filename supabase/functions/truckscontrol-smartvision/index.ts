...
serve(async (req) => {
  ...
  try {
    ...
    // RequestSmartVisionCam - Integração com câmeras MDVR
    let xmlRequest = `<?xml version="1.0" encoding="UTF-8"?>
<RequestSmartVisionCam>
  ...
  <senha>${escapeXml(TRUCKSCONTROL_PASSWORD)}</senha>`;
    ...
    if (input.dtInicio) {
      xmlRequest += `\n  <dtInicio>${escapeXml(input.dtInicio)}</dtInicio>`;
    }
    if (input.dtFim) {
      xmlRequest += `\n  <dtFim>${escapeXml(input.dtFim)}</dtFim>`;
    }
    
    xmlRequest += `\n</RequestSmartVisionCam>`;

    console.log("[truckscontrol-smartvision] start", { ts: new Date().toISOString(), veiID: input.veiID });
    ...
    const camNodes = parseXmlArray(responseText, "SmartVisionCam") || parseXmlArray(responseText, "Cam");
    const cameras = camNodes.map(xml => ({
      camID: parseXmlValue(xml, "camID"),
      veiID: parseXmlValue(xml, "veiID"),
      canal: parseXmlValue(xml, "canal"),
      dt: parseXmlValue(xml, "dt"),
      url: parseXmlValue(xml, "url"),
      thumb: parseXmlValue(xml, "thumb"),
    }));

    console.log("[truckscontrol-smartvision] finished", { count: cameras.length });
    ...
    console.error("[truckscontrol-smartvision] error:", err);
