...
serve(async (req) => {
  ...
  try {
    ...
    // RequestAcessorioVeiculo - Acessórios instalados por veículo
    const xmlRequest = `<?xml version="1.0" encoding="UTF-8"?>
<RequestAcessorioVeiculo>
  <login>${escapeXml(TRUCKSCONTROL_USER)}</login>
  <senha>${escapeXml(TRUCKSCONTROL_PASSWORD)}</senha>
  <todosItens>1</todosItens>
</RequestAcessorioVeiculo>`;

    console.log("[truckscontrol-acessorios-veiculo] start", { ts: new Date().toISOString() });
    ...
    const acessorioNodes = parseXmlArray(responseText, "AcessorioVeiculo");
    const acessorios = acessorioNodes.map(xml => ({
      veiID: parseXmlValue(xml, "veiID"),
      acvID: parseXmlValue(xml, "acvID"),
    }));

    console.log("[truckscontrol-acessorios-veiculo] finished", { count: acessorios.length });
    ...
    console.error("[truckscontrol-acessorios-veiculo] error:", err);
